...
func TestAutoResolution_ExactMatch(t *testing.T) {
	candidates := []resolver.Candidate{
		{IssuerKey: "aapl_us", CompanyName: "Apple Inc.", Symbol: "AAPL:US", Exchange: "NGS", FilingCount: 4127},
	}
...
func TestAutoResolution_Ambiguous(t *testing.T) {
	candidates := []resolver.Candidate{
		{IssuerKey: "apple_ca", CompanyName: "Apple Canada Ltd.", Symbol: "APL:TSX", Exchange: "TSX", FilingCount: 5},
		{IssuerKey: "aapl_us", CompanyName: "Apple Inc.", Symbol: "AAPL:US", Exchange: "NGS", FilingCount: 8},
	}
	body, _ := json.Marshal(candidates)
	fc := &fakeHTTPClient{response: body, status: http.StatusOK}
	...
	ambig, ok := err.(resolver.ErrAmbiguous)
	if !ok {
		t.Fatalf("expected ErrAmbiguous, got %T: %v", err, err)
	}
	if ambig.Query != "Apple" {
		t.Errorf("Query: want Apple, got %q", ambig.Query)
	}
	if len(ambig.Candidates) != 2 {
		t.Errorf("Candidates: want 2, got %d", len(ambig.Candidates))
	}
}

// TestAutoResolution_IssuerKeyBypass verifies that a value matching ^[a-z0-9_]+$
// bypasses AutoResolve without a network call.
func TestAutoResolution_IssuerKeyBypass(t *testing.T) {
	fc := &panicClient{}
	res, err := resolver.AutoResolve(context.Background(), fc, "aapl_us", "")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if res.IssuerKey != "aapl_us" {
