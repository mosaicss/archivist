...
// validateCascade converts spec rows/cols to cascade types and calls ValidateSpec.
func validateCascade(
	...
	var cascadeRows []cascade.Row
	for _, r := range spec.Rows {
		cascadeRows = append(cascadeRows, cascade.Row{
			Custom:    r.Custom,
			IssuerKey: r.Company,
		})
	}
	var cascadeCols []cascade.Column
	for _, c := range spec.Columns {
		cascadeCols = append(cascadeCols, cascade.Column{
			Name:   c.Name,
			Source: c.Source,
		})
	}

	violations := cascade.ValidateSpec(cascadeRows, cascadeCols)
	if len(violations) == 0 {
		return nil
	}
	...
	if formatFlag == "json" {
		...
		var vjs []violationJSON
		for _, viol := range violations {
			vjs = append(vjs, violationJSON{
				Row:     viol.Row,
				Col:     viol.Col,
				Rule:    viol.Rule,
